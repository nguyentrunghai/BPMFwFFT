...
if args.submit:
    ...
    for complex in complex_names:
        grid_sizes[complex] = get_grid_size_from_nc(os.path.join(grid_dir, complex, GRID_NC)) 
    complex_names.sort(key=lambda name: grid_sizes[name])
    print "Complex   grid size"
    for complex in complex_names:
        print complex, grid_sizes[complex]
    ...
    for complex in complex_names:
        ...
        grid_sub_dir   = os.path.join(grid_dir, complex)
        lig_ensemble_sub_dir = os.path.join(lig_ensemble_dir, complex)

        out_dir = os.path.abspath(complex)
        qsub_file = os.path.join(out_dir, id+"_fft.job")
        log_file  = os.path.join(out_dir, id+"_fft.log")
